...
cleaned <- ca_pums %>%
  ...
  filter(
    (SPORDER %in% 1), #assuming person 1 and 2 are the partners
...
```{r}
order <- c( "$10,000 or less", "$10,001 to $20000","$20,001 to $30,000","$30,001 to $40,000","$40,001 to $50,000","$50,001 to $75,000","$75,001 to $100,000","$100,001 to $150,000","$150,001 to $200,000","$200,001 or more")
```
...
```{r}
American_Indian <- cleaned %>%
  mutate(
    RAC1P = as.numeric(RAC1P)
  ) %>% 
  filter(
    (RAC1P %in% 3)) #American Indian
```
...
other <- cleaned %>%
  ...
  filter(
    (RAC1P %in% 8)) #Some Other Race alone
```
